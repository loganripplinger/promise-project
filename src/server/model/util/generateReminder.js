...
module.exports = {
  oneDayBeforeCourt: (person, objCourtDate) => {
    ...
    timeLocationMessage = `Hello ${
      person.name
    }, reminder that you have court tomorrow at ${formatter.time(
      datetime
    )} at ${objCourtDate.getLocation()}.`;
    assistanceMessage = `If you need assistance with transportation, please contact your case manager at ${
      person.caseManager.number
    }.`;
  ...
  courtDateConfirmation: (datetime, person, objCourtDate) => {
    confirmationMessage = `Hello ${
      person.name
    }, this is to confirm that Court Date has been added to your schedule for ${formatter.fancyTime(
      objCourtDate.getDatetime()
    )}. Take care.`;
